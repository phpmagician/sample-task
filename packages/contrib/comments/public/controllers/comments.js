...
angular.module('mean.comments').controller('CommentsController', ['$scope', '$http', 'Global', '$stateParams', 'Comments', 'FetchComments', 'mentionUser', 'MeanSocket', 'utils','MeanUser',
  function($scope, $http, Global, $stateParams, Comments, FetchComments, mentionUser, MeanSocket, utils,MeanUser) {
    ...
    $scope.global = Global;
    $scope.global.user = MeanUser.user;
    $scope.commentEditable = true;
    if($scope.global.user._id){

      $scope.isAdmin = $scope.global.user.roles.indexOf('admin') !== -1;
    ...
    $scope.create = function(body, parent, tagged_users) {
      ...
      comment.$save().then(function(data) {
        ...
        if ($scope.parent.comments.length === 0) {
         ...
         if($scope.isAdmin || $scope.global.user._id){

               $scope.parent.comments.push(data);
         }else{

                
              $scope.msg='comment send for approval';
